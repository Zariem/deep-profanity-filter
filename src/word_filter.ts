...
export const doesContainBadWords = (inputString: string, processedWordLists: ProcessedWordLists) => {
  if (findAnyBadWord(inputString, processedWordLists)) {
    return true;
  }
  return false;
};
...
export const findBadWordLocations = (
  ...
  { firstMatchOnly = false }: WordSearchOptions = {},
): BadWordMatchData[] => {
  ...
  for (const badwordData of processedWordLists.badWordData) {
    ...
    let locations = findBadWordMatchData(inputString, badwordData, processedWordLists.whitelistMap, false);
    ...
    if (badwordData.strictRegexp !== undefined) {
      ...
      const circumventions = findBadWordMatchData(inputString, badwordData, processedWordLists.whitelistMap, true);
 ...
 */
export const findAnyBadWord = (inputString: string, processedWordLists: ProcessedWordLists): string | undefined => {
  const result = findBadWordLocations(inputString, processedWordLists, { firstMatchOnly: true });
  if (result.length > 0) {
 ...
 * @returns The first bad word that was found in the input, or undefined if no bad word was found.
 */
export const findAllBadWords = (inputString: string, processedWordLists: ProcessedWordLists): string[] => {
  const badWordLocations = findBadWordLocations(inputString, processedWordLists);
